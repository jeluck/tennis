...
<link rel="stylesheet" href="/css/calendar.css?${getVersion }">
<link rel="stylesheet" href="/js/plugin/swiper.3.1.7.min.css">
<%@ include file="./common/common_init.jsp" %>
<script src="/js/plugin/swiper.3.1.7.min.js"></script>
<script src="/js/calendar.js?${getVersion }"></script>
<script src="/js/jquery-1.8.1.min.js"></script>
<script src="/js/admin_common.js"></script>
<script>
  var sLocalStorage = 'riChengBiaoZhuChang'; //app唯一标识，不可与其他页面重复，否则会报错
</script>
<!--本页面单独使用文件 end-->
</head>
<body class="lz-body">
          ...
          <div class="lz-date swiper-container-horizontal swiper-container-free-mode" id="lz-date-0" data-info="选择一周的某一天">
            <ul class="swiper-wrapper">
              <c:forEach items="${dateList }" var="d">
	        	<li onClick="lz.schedule(this,'${d.month}','${d.day }',${coach.id },'${d.week }',1,'${coach.coachType }','${d.year }',0);" class="swiper-slide">
    ...
    <div style="display:none;" data-info="预约收费"> 
      ...
      <ul class="lz-setcharge" id="lz-setcharge">
        ...
        <c:forEach items="${person }" var="p">
        	<c:if test="${p.price != 0 }">
	          	<li>
	          		<span class="lz-left"></i> 1对${p.people_num }</span><span class="lz-right">${p.price }/小时</span>
	          	</li>
          	</c:if>
          </c:forEach>
      </ul>
      ...
      <section class="lz-mb" style="display:none;">
        <div class="lz-destine">
          <div class="lz-date swiper-container-horizontal swiper-container-free-mode" id="lz-date-1" data-info="选择一周的某一天">
            <ul class="swiper-wrapper">
              <c:forEach items="${dateList }" var="d">
	        	<li onClick="lz.schedule(this,'${d.month}','${d.day }',${coach.id },'${d.week }',1,'${coach.coachType }','${d.year }',1);" class="swiper-slide">
	            <time>${d.year}&nbsp;-&nbsp;${d.month } &nbsp;-&nbsp;${d.day }</time>
	             ${d.week }</li>
        	</c:forEach>
            </ul>
