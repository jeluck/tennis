...
<%-- 			<label>微信号：</label><input type="text" name="wenumber" style="width: 110px; height: 25px;" value="${wenumber}" /> --%>
			...
			<a class="btn btn-sm btn-success" onclick="location.href = 'userlist.do?pagenumber=1';">[ 取消查询 ]</a></span>
		</form>
	</div>
	<div>
		<div class="col-xs-13">
					...
					<tbody>
						...
						<c:forEach items="${data_page.dataList }" var="weuser" varStatus="v">
							<tr>
								...
								<td>
									<c:if test="${weuser.idcard_status == 1 }" var="stat">已认证</c:if>
									<c:if test="${!stat }">未认证</c:if>
								</td>
<!-- 								<td> -->
<%-- 									<c:if test="${weuser.flag == 1 }" var="lock">未锁定</c:if> --%>
<%-- 									<c:if test="${!lock }">已锁定</c:if> --%>
<!-- 								</td> -->
								<td>
									<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
<%-- 										<c:if test="${weuser.flag == 1  }" var="lock"> --%>
<!-- 											<button class="btn btn-xs btn-danger" -->
<%-- 												onclick="location='lock_user_flag.do?pageNumber=${data_page.currentPage}&aid=${weuser.id }&flag=0'"> --%>
<!-- 												<i class="icon-lock bigger-120">锁定</i> -->
<!-- 											</button> -->
<%-- 										</c:if> --%>
<%-- 										<c:if test="${!lock }"> --%>
<!-- 											<button class="btn btn-xs btn-info" -->
<%-- 												onclick="location='lock_user_flag.do?pageNumber=${data_page.currentPage}&aid=${weuser.id }&flag=1'"> --%>
<!-- 												<i class="icon-unlock bigger-120">解锁</i> -->
<!-- 											</button> -->
<%-- 										</c:if> --%>
									</div>
									<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
										<button class="btn btn-xs btn-info"
											onclick="location='queryuser.do?&aid=${weuser.id }'">
									...
									<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
										<button class="btn btn-xs btn-info"
											onclick="location='/pgm/userorderList.do?userId=${weuser.id }'">
