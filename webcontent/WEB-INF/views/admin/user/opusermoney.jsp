...
<script type="text/javascript">
	...
	function go_page(pageNumber){
		location.href = "opusermoney.do?&pageNumber="+pageNumber;
	}
	//添加测试信息
	function addNotice(){
	...
	function recharge(id,pageNumber,_obj){
		var input = _obj.parentNode.getElementsByTagName('input')[0];
		if(input.value != null && input.value != "" && input.value > 0){
			location.href = "saveopusermoney.do?userId="+id+"&amount="+input.value+"&pageNumber="+pageNumber;
		}else{
			var span = _obj.parentNode.getElementsByTagName('span')[0];
			span.innerHTML ="请输入金额";
		}
	}
...
<body>
	<div class="breadcrumbs" id="breadcrumbs">
		...
		<ul class="breadcrumb" style="padding-top:8px;">
			...
			<li class="active">操作用户金额</li>
		</ul>
	</div>
	<div class="page-header">
		<h1>操作用户金额<span id="time" style="font-size:20px;float: right"></span></h1>
	</div>
	<div class="query_form">
		<form id="QueryForm" name="form1" class="form-inline" action="opusermoney.do;"
			  method="post">
						...
						<c:if test="${havadata }">
							...
							<c:forEach items="${data_page.dataList }" var="weuser" varStatus="v">
								<tr>
									...
									<td>${weuser.amount}</td>
									<td >
										<input style="IME-MODE: disabled; width: 110px; height: 25px; line-height:0.38rem;" 
										 onchange="clert(this)" onkeyup="clearNoNum(this)"
                						 maxlength="10"  />
									&nbsp;<a class="btn btn-sm btn-success" onclick="recharge(${weuser.id},${data_page.currentPage },this)">[ 充值 ]</a> 
	...
	<script>
paging($('.pagination')[0],'pagination','opusermoney.do?pageNumber=',${data_page.rowCount},${data_page.pageSize},${data_page.currentPage});
	</script>
</body>
</html>