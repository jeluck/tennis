...
<script type="text/javascript">
	...
	function go_page(pageNumber) {
		...
		document.form1.action = "userorderList.do";
	...
	function CancelQuery() {
		location.href = "userorderList.do?pagenumber=1&basestatus=${basestatus}&paystatus=${paystatus}&deliverystatus=${deliverystatus}";
	}
		...
		<form id="QueryForm" name="form1" class="form-inline"
			...
			<select type="text" name="playgroundId"
				...
				<c:forEach items="${playground }" var="p">
					...
					取消查询 ]</a></span> <span>历史总交易量:${count }</span><span>历史交易总额:${priceCount }</span>
		</form>
	</div>
							...
							<c:forEach items="${data_page.dataList }" var="order"
								...
								<tr>
									...
									<td><c:if test="${order.status == 1 }">未支付</c:if> <c:if
											...
											<c:if test="${order.status == 9 }">退款失败</c:if></td>
