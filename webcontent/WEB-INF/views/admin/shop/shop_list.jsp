...
<script type="text/javascript">
	...
	function go_page(pageNumber){
		location.href = "shoplist.do?&pageNumber="+pageNumber;
	}
...
<body>
	...
	<div class="query_form">
		<form id="QueryForm" name="form1" class="form-inline" action="shoplist.do;"
			  method="post">
			...
			<a class="btn btn-sm btn-success" onclick="location.href = 'shoplist.do?pagenumber=1';">[ 取消查询 ]</a></span>
		</form>
	</div>
	<div>
		<div class="col-xs-13">
			<div class="table-responsive">
				<table id="sample-table-1" class="table table-striped table-bordered table-hover">
					...
					<tbody>
						...
						<c:forEach items="${data_page.dataList }" var="shop" varStatus="v">
							<tr>
								<td>${v.index+1}</td>
								<td>${shop.shopName}</td>
								<td>${shop.shopPhone}</td>
								<td>${shop.realName}</td>
								<td>${shop.identification}</td>
								<td>
									<c:if test="${shop.isRealNameAuthentication == 0 }" var="lock">认证</c:if>
									<c:if test="${!lock }">未认证</c:if>
								</td>
								<td>
									<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
										<button class="btn btn-xs btn-info"
											onclick="location='queryshop.do?&sid=${shop.id }'">
	...
	<script>
paging($('.pagination')[0],'pagination','shoplist.do?pageNumber=',${data_page.rowCount},${data_page.pageSize},${data_page.currentPage});
	</script>
</body>
</html>