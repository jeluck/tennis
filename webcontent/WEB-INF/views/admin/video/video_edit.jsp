...
<body>
	...
	<div class="page-header">
		<h1>修改视频信息 <span id="time" style="font-size:20px;float: right"></span></h1>
	</div>

	<form id="QueryForm" name="form1" class="form-inline" action="edit_video.do"  
		...
		<table width="70%" border="0" align="center">
							...
							<td >
								<div class="col-sm-9">
									<input type="text"
										   ...
										   class="col-xs-10 col-sm-5" required value="${o.httpurl }" />
								</div>
							</td>
						</tr>
						<tr >
								...
								<div class="col-sm-9">
									<select name="video_classification_id.id" style="width: 360px;" class="col-xs-10 col-sm-5">
										...
										<c:forEach var="type" items="${videoType }">
											<option value="${type.id }" <c:if test="${type.id==1 }">selected="selected"</c:if> >${type.category_name }</option>
										</c:forEach>
									</select>
								</div>
							</td>
			...
			<tr>
				...
				<td width="40%">
					...
					<button class="btn btn-sm btn-success" type="button" onclick="location='video_list.do'"> [ 返回 ]
					</button>
				</td>
				<td width="20%">&nbsp;</td>
			</tr>
