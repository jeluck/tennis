...
<script type="text/javascript">
	...
	function go_page(pageNumber) {
		...
		document.form1.action = "canorderlist.do";
	...
	function CancelQuery() {
		location.href = "canorderlist.do?pagenumber=1&basestatus=${basestatus}&deliverystatus=${deliverystatus}";
	}
...
<body>
	...
	<div class="query_form">
		<form id="QueryForm" name="form1" class="form-inline"
			...
			</select> <span> <a class="btn btn-sm btn-success" onclick="Query();">[
							...
							<c:forEach items="${data_page.dataList }" var="order"
								...
								<tr>
									...
									<td><c:if test="${order.status == 1 }">未支付</c:if> <c:if
											...
											<c:if test="${order.status == 10 }">作废</c:if></td>
										...
										<c:if test="${order.status==1}">
											<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
												<button class="btn btn-xs btn-info"
													onclick="javascript:if(confirm('是否确认取消订单？')){canOrder('${order.orderMainNo}','${order.weuser.id }')}">
													<i class=" icon-eye-open bigger-120">取消订单</i>
												</button>
											</div>
											<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
												<button class="btn btn-xs btn-info"
													onclick="javascript:if(confirm('是否现场支付订单？')){livePayOrder('${order.orderMainNo}')}">
													<i class=" icon-eye-open bigger-120">现场支付</i>
												</button>
											</div>
										</c:if>
									</td>
	...
	<script type="text/javascript">
		function canOrder(orderMainNo,userId){
			var info=util.POST("/pgm/canOrder.do",{"orderMainId":orderMainNo,"userId":userId});
		...
		function livePayOrder(orderMainNo){
			var info=util.POST("/pgm/livePayOrder.do",{"orderMainId":orderMainNo});
	...
	<script>
paging($('.pagination')[0],'pagination','canorderlist.do?pagenumber=',${data_page.rowCount},${data_page.pageSize},${data_page.currentPage});
	</script>
</html>