...
<body>
	<div class="user">
		<span> <input type="file" name="imgFile"
			style="position: absolute; top: 37px; left: 130px; opacity: 0; height: 64px; width: 64px;" />

			<c:if test="${not empty agent.header }" var="hava">
				<img id="headerid" class="head-img" src="${agent.header}">
			</c:if> <c:if test="${!hava }">
				<img id="headerid" class="head-img" src="/image/nophoto_120X120.jpg" />
			</c:if>
		</span><br> <span><a class="userName">${agent.agentCompany.company_name}</a><br>
			<!--     <a class="userFrom">微信用户</a> --> </span>
	</div>
	<div class="mySet myPocket">
		<div class="choice-wechat01">
			<!--   <span class="Count-img"></span>-->
			<span class="Count-text"
				style="margin-top: 15px; line-height: 40px; margin-left: 0%;">我的钱包（元）
				<a href="countDetali.do" class="arrowRight">&nbsp;资金管理/明细&nbsp;></a><br>
				<a class="nowPocket">${agent.agentCompany.amount}</a>
			</span><br>
	...
	<div class="mySet">
		<div class="choice-wechat01"
			onclick="window.location.href='tomyProducts.do'">
			<span class="Count-img01"></span> <span class="Count-text"><a
				href="tomyProducts.do">商品管理</a></span> <a href="tomyProducts.do"> <span
				class="arrowRig-img"></span></a>
		</div>
	</div>
	<div class="mySet">
		<div class="choice-wechat01" onclick="window.location.href='toset.do'">
			<span class="Count-img02"></span> <span class="Count-text"><a
				href='toset.do'>设置</a></span> <a href="toset.do"> <span
				class="arrowRig-img"></span></a>
		</div>
	</div>
	
	<div class="mySet" style="height: 100px;"></div>
		...
		$('input[type=file]')
				.each(
						function(i, o) {
							$('body')
									.append(
											'<div class="upfiles"><form class="imgform'+i+'" target="frameimg'+i+'" method="post" enctype="multipart/form-data" action="agent_upload_head.do">'
													+ '<input type="file" accept="image/jpe,image/jpg,image/jpeg,image/gif,image/png" name="'
													+ o.name
													+ '" onclick="uploadimg();" class="inimg'
													+ i
													+ '" style="position: absolute; top: 37px; left: 130px; opacity: 0; height: 64px; width: 64px;">'
							...
							$('.inimg' + i)
									...
									.change(
											function(e) {
												...
												$('[name=frameimg' + i + ']')[0].onload = function(
														rs) {
													rs = fr.document.body.innerHTML;
													$(o.parentNode.parentNode)
															.find('img').eq(0)
															.attr('src',
																	'/' + rs);//.height(198).width(198);
												};
												files = e.target.files[0];
												imgform.submit();
											});
