...
function eachRighs(right_data){
 	var righthtml = '<div class="app"><p><input type="checkbox" class="btn btn-sm btn-success " onclick="check_all(this)" value="全选" /><strong>网球馆-后台管理权限</strong></p>';
 ...
 * ---- 角色绑定用户  ------------------- 华丽的分割线 ------------------------
 */
 function beforeEditRoleUsers(role_code){
	 op_rolecode = role_code;
	 var data = util.GET("get_role_users.do",{"role_code":role_code}); 
...
function each_option(data){
	...
	$.each(data,function(i,item){
		option_html += '<option value="'+item.id+'">'+item.username+'&nbsp;&nbsp;-&nbsp;&nbsp;'+item.mobile+'</option>';
	});
	return option_html;
}
//移动
...
function saveRoleUsers(){
	...
	info = util.POST("save_role_users.do",{"role_code":op_rolecode,"users":users});
	if(info.status){
		parent.ShowMsg("操作提示：",info.info);
...
function checkRole_code(){
	var info = util.POST("checkRole_code.do",{"role_code":$("#edit_role_code").val(),"id":$("#edit_id").val()});
	if(info<=0){
									...
									<td>
										<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
											...
											<button class="btn btn-xs"
												onclick="beforeEditRoleUsers('${role.role_code}');">
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<input type="text" name="role_code" id="edit_role_code" style="width: 320px;" placeholder="角色标识为小写字母"
								...
								<span id="role_code_erroe" style="color: red;"></span><span style="color: red;">*</span>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right"
						...
						<div class="col-sm-9">
							<textarea rows="6" cols="50" name="role_desc" required></textarea><span style="color: red;">*</span>
						</div>
					</div>
