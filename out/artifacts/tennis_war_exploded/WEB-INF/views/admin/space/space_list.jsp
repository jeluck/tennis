...
<body>
	...
	<div class="query_form">
		<form id="QueryForm" name="form1" class="form-inline" action="playground_list.do;"
			...
			<label>场地名称：</label><input type="text" name="name" style="width: 110px; height: 25px;" value="${company_name}" />
			&nbsp;
			...
			<a class="btn btn-sm btn-success" onclick="location.href = 'toaddspace.do?oid=${oid }&is_play=is_play';">[ 添加 ]</a></span>
			</c:if>
			<c:if test="${empty is_play }">
			<a class="btn btn-sm btn-success" onclick="location.href = 'toaddspace.do?oid=${oid }';">[ 添加 ]</a></span>
			</c:if>
		</form>
	</div>
	<div>
								...
								<td>
									<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
										<button class="btn btn-xs btn-info"
											onclick="location='toedit_space.do?oid=${o.id }&check=check&playgroundId=${oid }'">
											<i class=" icon-eye-open bigger-120">查看</i>
										</button>
										<button class="btn btn-xs btn-info"
											onclick="location='toedit_space.do?oid=${o.id }&playgroundId=${oid }'">
											<i class=" icon-edit bigger-120">修改</i>
										</button>
										<button class="btn btn-xs btn-info"
											onclick="location='toeditspace_time_price.do?spaceId=${o.id }&playgroundId=${oid }'">
