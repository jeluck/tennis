...
<script type="text/javascript">
	...
	function go_page(pageNumber){
		location.href = "brandInfolist.do?&pageNumber="+pageNumber;
	}
...
<body>
	...
	<div class="page-header">
		<h1>品牌列表 <span id="time" style="font-size:20px;float: right"></span></h1>
	</div>
	<div class="query_form">
		<form id="QueryForm" name="form1" class="form-inline" action="brandInfolist.do;"
			  method="post">
			...
			<a class="btn btn-sm btn-success" onclick="location.href = 'brandInfolist.do?pagenumber=1';">[ 取消查询 ]</a></span>
		</form>
	</div>
	<div>
		<div class="col-xs-13">
			<div class="table-responsive">
				<table id="sample-table-1" class="table table-striped table-bordered table-hover">
					...
					<tbody>
						...
						<c:forEach items="${data_page.dataList }" var="brandInfo" varStatus="v">
							<tr>
								<td>${v.index+1}</td>
								<td>${brandInfo.brand_name}</td>
								<td>${brandInfo.create_time}</td>
								<td>
									<c:if test="${brandInfo.flag == 0 }" var="lock">可用</c:if>
									<c:if test="${!lock }">停用</c:if>
								</td>
								<td>
									<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
										<button class="btn btn-xs btn-info"
											onclick="location='querybrandInfo.do?&oid=${brandInfo.id }'">
											<i class=" icon-eye-open bigger-120">查看</i>
										</button>
										<button class="btn btn-xs btn-info"
											onclick="location='toeditbrandInfo.do?&oid=${brandInfo.id }'">
	...
	<script>
paging($('.pagination')[0],'pagination','brandInfolist.do?pageNumber=',${data_page.rowCount},${data_page.pageSize},${data_page.currentPage});
	</script>
</body>
</html>